...
st.title("사운드볼텍스 18레벨 추천기")
st.markdown("### 학번: 2023204058 / 이름: 이재현")
st.divider()

# 사용자 입력
st.subheader("선호하는 곡 조건 설정")
...
with col2:
    ...
    user_patterns = st.multiselect("선호 핵심 패턴 (1개 이상)", pattern_list, default=["노브"])

st.write("") 

# 추천 요청
if st.button("곡 추천받기", use_container_width=True):
    if not user_patterns:
        st.warning("선호 패턴을 최소 1개 이상 선택해주세요.")
    else:
        req_data = {
            "min_lv": lv_range[0],
            "max_lv": lv_range[1],
            ...
            if data["status"] == "success":
                ...
                for i, s in enumerate(songs):
                    with cols[i]:
                        ...
                        st.caption(f"특화 패턴: {', '.join(s['patterns'])}")
            else:
